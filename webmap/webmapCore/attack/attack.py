...
from webmapCore.file.auxtext import AuxText
import requests
import sys
...
class Attack(object):
    ...
    if hasattr(sys, "frozen"):
        ...
        BASE_DIR = os.path.dirname(sys.modules['webmapCore'].__file__)
    ...
    def attack(self, http_resources, forms):
        if self.doGET is True:
            for http_res in http_resources:
                ...
                if self.verbose == 1:
                    self.log(("+ attackGET {0}"), url)

                try:
                    self.attackGET(http_res)
                except socket.error, se:
                    self.log(('error: {0} while attacking {1}'), repr(str(se[0])), url)
                except requests.exceptions.Timeout:
                    self.log(('error: timeout while attacking {0}'), url)
                except Exception, e:
                    self.log(('error: {0} while attacking {1}'), repr(str(e[0])), url)

        if self.doPOST is True:
            for form in forms:
                if self.verbose == 1:
                    self.log(("+ attackPOST {0} from {1}"), form.url, form.referer)
                ...
                try:
                    ...
                    print(('error: timeout while attacking {0}').format(url))
         #       except Exception, e:
         #           self.log(('error: {0} while attacking {1}'), repr(str(e[0])), url)