...
class mod_file(Attack):
    ...
    warnings_desc = [
            # Vulnerabilities
            ("<title>Google</title>",                 ("Remote inclusion vulnerability"), 1),
            ("root:x:0:0",                            ("Linux local file disclosure vulnerability"), 1),
            ("root:*:0:0",                            ("BSD local file disclosure vulnerability"), 1),
            ("[boot loader]",                         ("Windows local file disclosure vulnerability"), 1),
            ("s:12:\"pear.php.net\";",                ("File disclosure vulnerability in include_path"), 1),
            ("PHP Extension and Application Reposit", ("File disclosure vulnerability in include_path"), 1),
            ("PEAR,&nbsp;the&nbsp;PHP&nbsp;Extensio", ("highlight_file() vulnerability in basedir"), 1),
            ("either use the CLI php executable",     ("include() of file in include_path"), 1),
            # Warnings
        ...
        for pattern, description, level in self.warnings_desc:
            if pattern in data:
                if level == 1:
                    ...
                    if warn == 0:
                        err_msg = ("Possible {0} vulnerability").format(description)
                        warn = 1
                        break
                ...
                if url not in self.attackedGET:
                    ...
                    if err != "":
                        self.logVuln(category=Vulnerability.FILE_HANDLING,
                                     ...
                                     request=evil_req,
                                     info=("{0} via injection in the query string").format(err))
                ...
                if attack_pattern not in self.attackedPOST:
                    ...
                    for payload in self.payloads:
                        ...
                        try:
                            ...
                            err, inc, warn = self.__findPatternInResponse(data, warn)
                        if err != "":
                            info_msg = ("{0} via injection in the parameter {1}")
                            self.logVuln(category=Vulnerability.FILE_HANDLING,
                                         level=Vulnerability.HIGH_LEVEL,
                                         request=evil_req,
                                         parameter=param_name,
