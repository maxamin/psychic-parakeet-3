...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        // Advance time 5 days so that attacker can get reward
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days

        const Attacker = await ethers.getContractFactory('AttackerTheRewarder', attacker);
        attackerContract = await Attacker.deploy(this.liquidityToken.address, this.rewardToken.address, this.rewarderPool.address, this.flashLoanPool.address);
