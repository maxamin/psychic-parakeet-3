...
startRpc(){
  ...
  while [[ $i -le $totalNodes ]]; do
    ...
    if tmux has-session -t node$i > /dev/null 2>&1; then
        ...
        tmux send-keys -t node$i " ./bin/geth --datadir ./chaindata/node$i --networkid $CHAINID --ws --ws.addr $IP --ws.origins '*' --ws.port 8545 --http --http.port 80 --rpc.txfeecap 0  --http.corsdomain '*' --nat 'any' --http.api db,eth,net,web3,personal,txpool,miner,debug --http.addr $IP --http.vhosts=$VHOST --txpool.lifetime 1h --vmdebug --pprof --pprof.port 6060 --pprof.addr $IP --syncmode full --gcmode=archive  --ipcpath './chaindata/node$i/geth.ipc' console" Enter
...
startValidator(){
  ...
  while [[ $i -le $totalValidator ]]; do
    ...
    if tmux has-session -t node$i > /dev/null 2>&1; then
        ...
        tmux new-session -d -s node$i
        tmux send-keys -t 0 "./bin/geth --datadir ./chaindata/node$i --networkid $CHAINID --bootnodes $BOOTNODE --mine --port 326$j --nat extip:$IP  --unlock 0 --password ./chaindata/node$i/pass.txt --syncmode=fast console" Enter
    fi
